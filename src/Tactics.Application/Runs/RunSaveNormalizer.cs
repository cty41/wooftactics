...
internal static class RunSaveNormalizer
{
    public static PureRunSaveSnapshot Normalize(PureRunSaveSnapshot snapshot)
    {
        ...
        if (setup is not null && (setup.CurrentCharacterIndex < 0 || setup.CurrentCharacterIndex > 3 ||
            setup.Choices.Count != setup.CurrentCharacterIndex || setup.SelectedCharacterIds.Count is < 0 or > 3 ||
            setup.SelectedCharacterIds.Count is 1 or 2 && (setup.CurrentCharacterIndex != 0 || setup.Choices.Count != 0) ||
        ...
        return snapshot with
        {
            ...
            // Party selection and starting-skill choices are ordered transaction logs, not sets.
